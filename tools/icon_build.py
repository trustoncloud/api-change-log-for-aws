...
def main(source, destination, size):
    ...
    icons = {}
    used = set()
    icon_2_service = {v: k for k, v in ICON_SERVICE_MAP.items()}
    ...
    if set(icon_2_service).difference(used):
        print(
            "missing service icons %s"
            % (", ".join(set(icon_2_service).difference(used)))
        )
    print("copied %d icons" % count)
    with (destination / "icons.css").open("w") as fh:
        icons = {k: "icons/%s.png" % v.lower() for k, v in ICON_SERVICE_MAP.items()}
        fh.write(
            Template(CSS_BUILD, lstrip_blocks=True, trim_blocks=True).render(
                icons=icons
            )
        )

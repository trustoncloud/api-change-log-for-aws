...
from .icons import ICON_SERVICE_MAP
...
def group_by_date(commits: List[Commit], year: bool = False, month: bool = False) -> List[Commit]:
    if year:
...
class TemplateAPI:
    ...
    def get_icon_style(self, name, size=64):
        icon_name = ICON_SERVICE_MAP.get(name)
        if not icon_name:
            return self.get_icon_style("aws", size)
        else:
            return f"icon {name}-{size}"
    ...
    def _get_service_model(self, service_change):
        ...
        if service_change.model_file == GIT_EMPTY_FILE:
            return
        data = json.loads(self.repo[service_change.model_file].read_raw().decode("utf8"))
        m = ServiceModel(data)
...
class Site:
    ...
    def copy_assets(self):
        if not self.assets_dir:
            return
        for atype in ("css", "js", "icons"):
            shutil.copytree(
                self.assets_dir / atype, self.output / "assets" / atype, dirs_exist_ok=True
            )
    ...
    def render_page(self, path, template: Optional[str] = None, force: bool = False, **kw):
        if template:
    ...
    def build_feed(self, commits: List[Commit]):
        ...
        for c in commits:
            for s in c.service_changes:
                ...
                fe.link({"href": self.link("archive/changes/%s-%s.html" % (c.id[:6], s.name))})
    ...
    def build_search_index(self, commits: List[Commit]):
        ...
        self.render_page("search_data.json", content=json.dumps(sd, cls=DateTimeEncoder))
        self.render_page("search/index.html", "search.jinja2")
    ...
    def build_service_pages(self, commits: List[Commit], services=None):
        ...
        for svc_name in sorted(groups):
            if services and svc_name not in services:
                continue
            svc_title = next(iter(groups[svc_name][0].select(svc_name))).title
            self.render_page(
    ...
    def build_index_pages(self, commits: List[Commit]):
        ...
        for idx, batch in [(0, commits)]:
            ...
            self.render_page(p, "index.jinja2", releases=batch, pager=pager, force=True)
